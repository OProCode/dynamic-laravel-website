<x-app-layout>
    ...
    <form method="POST"
        ...
        <div class="flex flex-row w-full">
            ...
            <input id="Name" name="name" type="text" class="p-2 w-full bg-gray-400 dark:bg-gray-800 border-none hover:bg-gray-500 focus:bg-gray-700" value="{{old('name') ?? $word->name}}"/>
        </div>

        <div class="flex flex-row w-full">
            <label for="Code" class="p-2 w-1/6 bg-gray-400 dark:bg-gray-600 rounded-l-md">{{__("WORD TYPE")}}</label>
            <select id="Code" name="word_type_id" class="p-2 w-full bg-gray-400 dark:bg-gray-800 border-none hover:bg-gray-500 focus:bg-gray-700">
                @foreach ($wordTypes as $wordType)
                    <option value="{{ $wordType->id }}" @if($wordType->id == $word->word_type_id) selected @endif>
                        {{$wordType->code}}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex flex-row w-full text-center pt-10 text-2xl">
            ...
            <a href="{{route('words.show', ['word'=>$word])}}" class="grow">
            ...
            <a href="{{ route('words.index', ['word'=>$word])}}" class="grow">
